...
def analyze_image(interpreter, image):
    ...
    output_category = np.squeeze(interpreter.get_tensor(output_details[0]["index"]))
...
img = cv2.imread("data/images/div2k/0010.png")
input_data = cv2.resize(img, (width, height))
input_data = np.expand_dims(input_data, axis=0)
print(input_data.shape)

print(input_data[0]/255)
input_data = input_data/255
print("ooooooooooooooooooooo")

category = analyze_image(interpreter, input_data)
...
predicted_id = np.argmax(np.abs(category[0]), axis=-1)
print(predicted_id)
print(category[0][predicted_id])

category_label = labels[predicted_id]

# odds = math.exp(category[0][predicted_id])
# print(odds)
# prob = 1 / (1+odds)
# print(prob)

print(category[0][predicted_id], category_label)
